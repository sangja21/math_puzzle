...
- 선정 퍼즐: **후보 1 — 체스판 쌀알 시뮬레이터** (2026-05-12 결정)
- 스토리 톤: 인도 왕 ↔ 노현자 전설을 페이지 정체성으로 확장
  - `StoryHeader`: 전설 3~4문장 + 이모지 ♟️ + 코어 수학 칩 [`2ⁿ`, `로그`, `자연상수 e`, `로그 스케일`]
  - §1 Intro: 한 단락 + **자동재생 쌀 박스** 한 컷 (마일스톤 카드 표는 폐기, 시각으로 대체)
  - 게임 탭: 사용자가 *다음 칸* 버튼으로 직접 진행, 박스 위에 *왕↔현자 말풍선* 3단계 진화 (1·32·64칸)
  - 챌린지 카드: 같은 인도 톤 (쌀 1톤 / 왕국 / 세계 생산량)
- 게임 탭 시각화 **— 떨어지는 쌀 박스**
  - 우측 박스 (320×400px 권장) 안에 쌀알이 위에서 *떨어져* 바닥에 쌓임
  - 매 칸마다 `2^(n-1)` 개의 알 추가 (현재 척도 단위로 환산)
  - 박스가 ~80% 차면 **카메라 줌아웃 + 척도 격상** (1알 단위: 1 → 100 → 만 → 억 → 조 → 경)
  - 64칸 도달 시 박스 밖으로 *쏟아지는* 마지막 컷
  - 한 칸당 떨어지는 알 캡: 시각 최대 ~300알, 초과분은 즉시 누적 (성능)
- 원리 탭 6섹션
  1. Intro — 전설 살붙임 + 자동재생 쌀 박스 (좌 텍스트 / 우 박스)
  2. 두 배의 폭증 — 1·2·4·8…64까지 막대 그래프 (linear / log 토글)
  3. 로그 = 거꾸로 — "몇 번 곱했나" 슬라이더 (10ⁿ ↔ log₁₀)
  4. 밑 비교 — 2ⁿ / 10ⁿ / eⁿ 동시 그래프 (`CoordPlane` 재사용)
  5. 자연 속의 로그 — 데시벨·리히터·pH·별 등급 카드 4장
  6. 자가 점검 — 3문항
- 게임 탭 컨트롤: `다음 칸` / `처음으로` / `자동 64칸 (0.3s/칸)`
- 챌린지 카드 (게임 탭 하단, 예측 → 공개 형)
  - "쌀 1톤(≈ 4천만 톨)을 처음 넘기는 칸은?" → 26칸
  - "한국 1년 쌀 생산량(≈ 350만 톤)을 넘기는 칸은?" → 48칸
  - "64칸 전체의 쌀을 지구 표면에 깔면 몇 m?" → 약 9m
- 신규 자산
  - 체스판 SVG (8×8, exp-log 전용)
  - `RiceBox` 컴포넌트 (떨어지는 쌀 + 척도 격상 + 줌아웃)
  - `CoordPlane.yScale: 'linear' | 'log'` prop (§2·§4에서 사용)
  - 비유 카드 데이터 (라이브러리 상수: 한 술/한 가마/트럭/도시/왕국/세계 임계값)
- 라우트: `/puzzle/exp-log`
- 카드: `exp_log`, lessonGroup `함수와 그래프`, 이모지 ♟️
- 구현 시작일: 2026-05-12
